...
void Renderer::DrawScene()
{
	...
	glm::mat4 Transrate_x = glm::mat4(1.0f); // 단위 행렬로 초기화
	glm::mat4 Transrate_y = glm::mat4(1.0f);
	glm::mat4 Scale_down = glm::mat4(1.0f); // 단위 행렬로 초기화
	...
	if (transform_type == 0)
	{
		Transrate_x = glm::translate(Transrate_x, glm::vec3(move_size_x, move_size_z, 0.0f)); // 
		Scale_down = glm::scale(Scale_down, glm::vec3(0.2f, 0.2f, 0.2f));
		Trans_form_sum = Transrate_x * Transrate_y* Scale_down * Rotate_x * Rotate_y;
		glUniformMatrix4fv(modelLocation, 1, GL_FALSE, glm::value_ptr(Trans_form_sum)); // 적용
